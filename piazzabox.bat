...
if %Var1%==1 (
	start cmd /C "title Cloning piazza repositories & echo Cloning piazza repositories... & echo. & cd %LOCAL_PIAZZA_REPO_PATH% & git clone https://github.com/venicegeo/pz-gateway.git & git clone https://github.com/venicegeo/pz-ingest.git & git clone https://github.com/venicegeo/pz-access.git & git clone https://github.com/venicegeo/pz-jobmanager.git & git clone https://github.com/venicegeo/pz-search-metadata-ingest.git & git clone https://github.com/venicegeo/pz-search-query.git & git clone https://github.com/venicegeo/pz-servicecontroller.git & git clone https://github.com/venicegeo/kafka-devbox.git & git clone https://github.com/venicegeo/pz-logger.git & git clone https://github.com/venicegeo/pz-workflow.git & git clone https://github.com/venicegeo/pz-jobcommon.git & echo. & cd pz-gateway & echo pz-gateway update & git pull & echo. & cd ../pz-access & echo pz-access update & git pull & echo. & cd ../pz-ingest & echo pz-ingest update & git pull & echo. & cd ../pz-jobmanager & echo pz-jobmanager update & git pull & echo. & cd ../pz-search-metadata-ingest & echo pz-search-metadata-ingest update & git pull & echo. & cd ../pz-search-query & echo pz-search-query update & git pull & cd ../pz-servicecontroller & echo pz-servicecontroller update & git pull & echo. & cd ../pz-workflow & echo pz-workflow update & git pull & echo. & cd ../pz-logger & echo pz-logger update & git pull & echo. & cd ../kafka-devbox & echo kafka-devbox update & git pull & echo. & cd ../pz-jobcommon & echo pz-jobcommon update & git pull &	echo. & pause" 
)
...
if %Var1%==3 (
	...
	rem wait couple of seconds before starting the workflow, logger needs to be running.
	rem timeout /t 30
...
if %Var1%==4 (
	start cmd /C "title PZ-GATEWAY & echo. & cd %LOCAL_PIAZZA_REPO_PATH% & echo Starting pz-gateway... & cd pz-gateway & java -jar target/piazza-gateway-1.0.0.jar --access.prefix=localhost --search.url=http://localhost:8581 --jobmanager.prefix=localhost --servicecontroller.port=8088 --servicecontroller.prefix=localhost --servicecontroller.protocol=http --logger.url=http://192.168.46.46:14600 --workflow.url=http://192.168.50.50:14400 --ingest.url=http://localhost:8084 --access.url=http://localhost:8085 & pause"
	start cmd /C "title PZ-INGEST & echo. & cd %LOCAL_PIAZZA_REPO_PATH% & echo Starting pz-ingest... & cd pz-ingest & java -jar target/piazza-ingest-1.0.0.jar --search.url=http://localhost:8580 --logger.url=http://192.168.46.46:14600 --workflow.url=http://192.168.50.50:14400 --access.url=http://localhost:8085 & pause"
 	start cmd /C "title PZ-ACCESS & echo. & cd %LOCAL_PIAZZA_REPO_PATH% & echo Starting pz-access... & cd pz-access & java -jar target/piazza-access-1.0.0.jar --logger.url=http://192.168.46.46:14600 & pause"
 	start cmd /C "title PZ-JOBMANAGER & echo. & cd %LOCAL_PIAZZA_REPO_PATH% & echo Starting pz-jobmanager... & cd pz-jobmanager & java -jar target/piazza-jobmanager-1.0.0.jar --logger.url=http://192.168.46.46:14600 & pause"
 	start cmd /C "title PZ-SEARCH-METADATA-INGEST & echo. & cd %LOCAL_PIAZZA_REPO_PATH% & echo Starting pz-search-metadata-ingest... & cd pz-search-metadata-ingest & java -jar target/pz-search-metadata-ingest-1.0.0.jar --logger.url=http://192.168.46.46:14600 & pause"
	start cmd /C "title PZ-SEARCH-QUERY & echo. & cd %LOCAL_PIAZZA_REPO_PATH% & echo Starting pz-search-query... & cd pz-search-query & java -jar target/pz-search-query-1.0.0.jar --logger.url=http://192.168.46.46:14600 & pause"
 	start cmd /C "title PZ-SERVICECONTROLLER & echo. & cd %LOCAL_PIAZZA_REPO_PATH% & echo Starting pz-servicecontroller... & cd pz-servicecontroller & java -jar target/piazzaServiceController-1.0.0.jar --workflow.url=http://192.168.50.50:14400 --logger.url=http://192.168.46.46:14600 --search.protocol=http --search.prefix=localhost --search.port=8581 --metadata.ingest.protocol=http --metadata.ingest.prefix=localhost --metadata.ingest.port=8580 & pause"
)

rem Gracefully shutdown of all running vagrant services created by piazza toolkit
if %Var1%==5 (
	rem stopping kafka / mongodb / geoserver / postgis / elasticsearch / logger / workflow
...
if %Var1%==h (
	start cmd /C "title Piazza Help & echo Following must be installed on the system: & echo ------------------------------------------ & echo -Vagrant: https://www.vagrantup.com/ if needed, run cmd as an admin and install: 'vagrant plugin install vagrant-hostsupdater' & echo --Change write permission on windows HOST file if you are on Windows 10. & echo -Oracle VM VirtualBox 5.0.10: https://www.virtualbox.org/ & echo -Maven 3.3.x: https://maven.apache.org/ & echo -Java jdk1.8.x from http://www.oracle.com & echo -Curl must be installed for windows users: https://curl.haxx.se/download.html &  echo. & echo. & echo Following environment variables should be set: & echo ---------------------------------------------- & echo JAVA_HOME Ex: JAVA_HOME=C:\Program Files\Java\jdk1.8.0_101 & echo vcap.services.pz-blobstore.credentials.access_key_id & echo vcap.services.pz-blobstore.credentials.secret_access_key & echo **You may also set the LOCAL_PIAZZA_REPO_PATH environment variable & echo    to point to preferred local directory containing existing repositories & echo. & echo. & echo Piazza Toolkit steps accomplish the following: & echo step 1: Will clone/pull repositories using git to a folder specified in LOCAL_PIAZZA_REPO_PATH environment variable. & echo step 2: Builds all cloned piazza repositories with maven. Maven must be installed and set in PATH. & echo step 3: Starts all required services MongoDB/ElasticSearch/PostGIS/GeoServer/Kafka/Logger/Workflow in individual vagrant managed local VMs. & echo step 4: Starts all java piazza apps locally on a new command/terminal window. Console output can be used for debugging purposes. Invididual app can be killed and restarted manually. & echo step 5: Gracefully stops local running service VMs MongoDB/ElasticSearch/PostGIS/GeoServer/Kafka/Logger/Workflow. Must be done before shutting down developer machine. & echo step 6: Destroys all local service VMs MongoDB/ElasticSearch/PostGIS/GeoServer/Kafka/Logger/Workflow. & echo step 7: Lists and prunes all global vagrant boxes existing on the developers host machine. & pause"
)
